...
import { fmtNum, pct, cls } from "@/lib/format";
...
function RecommendationPanel({ title, code, rows, consensus }: { title: string; code: string; rows: any[]; consensus: Record<string, string> }) {
  return <TerminalPanel title={title} code={code}>{rows.length === 0 ? <div className="terminal-empty">NO QUALIFIED SETUP</div> : <div className="recommendation-list">{rows.map((r) => <article className="recommendation-row" key={`${r.method}-${r.symbol}-${r.strategy}`}><div className="recommendation-symbol"><Link href={`/stock/${r.symbol}`} className="ticker-link">{r.symbol}</Link><span>{r.strategy}</span></div><div className="recommendation-price"><span>LAST</span><strong>{fmtNum(r.current_price)}</strong><span className={cls(r.action === "BUY" ? 1 : -1)}>{r.action}</span></div><div className="recommendation-levels"><span>ENTRY <b>{fmtNum(r.entry_low)}–{fmtNum(r.entry_high)}</b></span><span>TP1 <b className="positive">{fmtNum(r.tp1)}</b></span><span>TP2 <b className="positive">{fmtNum(r.tp2)}</b></span><span>SL <b className="negative">{fmtNum(r.stop_loss)}</b></span><span>R/R <b>{r.risk_reward ? Number(r.risk_reward).toFixed(2) : "—"}</b></span><span>SCORE <b>{r.score ?? "—"}</b></span><span>CONSENSUS <b>{consensus[r.symbol] || "NOT ANALYZED"}</b></span></div><div className="recommendation-why">{(r.reasons?.positive || []).map((x: string) => <span key={x} className="positive">+ {x}</span>)}{(r.reasons?.negative || []).map((x: string) => <span key={x} className="negative">− {x}</span>)}</div></article>)}</div>}
  </TerminalPanel>;
}

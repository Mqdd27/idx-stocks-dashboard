...
export function DataState({
  ...
  state: "live" | "delayed" | "closed" | "error";
  children: ReactNode;
}) {
  return (
    <span className={`data-state ${state}`}>
      <i />
      {children}
    </span>
  );
...
export function SectionHeading({
  ...
  return (
    <div className="terminal-page-head">
      <div>
        {eyebrow && <span className="terminal-eyebrow">{eyebrow}</span>}
        <h1>{title}</h1>
        {meta && <div className="terminal-page-meta">{meta}</div>}
      </div>

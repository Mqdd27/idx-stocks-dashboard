...
export default function Shell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      className={`app-shell terminal-shell ${collapsed ? "nav-collapsed" : ""}`}
    >
      <aside className="sidebar terminal-sidebar">
        <div className="sidebar-brand terminal-brand">
          <img className="brand-mark" src="/icon.svg" alt="STX Stocks IDX" />
          <div>
            <h1>STOCKS IDX</h1>
            <small>INDONESIA STOCKS</small>
          </div>
        </div>
        <button
        ...
        <nav className="nav terminal-nav" aria-label="Primary">
          {NAV.map((item) => (
            ...
            >
              <span className="nav-code">{item.key}</span>
              <span className="nav-label">{item.label}</span>
              {item.hint && <kbd>{item.hint}</kbd>}
            </Link>
          ))}
        </nav>
        <div className="sidebar-footer terminal-sidebar-footer">
          <span>DATA</span>
          <b>YF / GNEWS</b>
          <span>AI</span>
          <b>9ROUTER</b>
      ...
      <div className="main terminal-main">
        <header className="topbar terminal-command-bar">
          ...
          <MarketStatusChip />
          <span className="terminal-clock" aria-label="Waktu Jakarta">
            JKT {clock || "--:--:--"}
          </span>
          <span
            className={`connection-state ${health?.status === "ok" ? "ok" : "error"}`}
            title="Backend / database / 9Router status"
          >
            {health?.status === "ok" ? "SYS OK" : "SYS --"}
          </span>
          <AdminUnlock />
          <ModelSelector />
        </header>
  ...
  return (
    <div className="search-box terminal-search">
      ...
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" && results[0]) navigate(results[0].symbol);
          if (e.key === "Escape") {
            setOpen(false);
      ...
      {open && results.length > 0 && (
        <div className="search-results terminal-search-results">
          {results.map((r) => (
            <button key={r.symbol} onMouseDown={() => navigate(r.symbol)}>
...
function MarketStatusChip() {
  ...
  const state = status?.is_open
    ? "open"
    : status?.status === "BREAK"
      ? "break"
      : ["PUBLIC_HOLIDAY", "EXCHANGE_HOLIDAY", "WEEKEND"].includes(
            status?.status,
          )
        ? "holiday"
        : "closed";
  return (
    <span
      className={`terminal-market-badge ${state}`}
      title={status?.reason || "IDX market status"}
    >
      IDX {status?.status || "--"}
    </span>
  );
...
function AdminUnlock() {
  ...
  return (
    <button
      ...
      onClick={authenticated ? undefined : unlock}
      title={
        authenticated ? "Admin session active" : "Unlock protected actions"
      }
    >

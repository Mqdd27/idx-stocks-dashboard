...
const GET_CHANNEL =
  typeof window !== "undefined" && "BroadcastChannel" in window
    ? new BroadcastChannel("stocks-api-cache")
    : null;
GET_CHANNEL?.addEventListener("message", (event) => {
  const { url, value, expires } = event.data || {};
  if (url && expires && value !== undefined)
    GET_CACHE.set(url, { expires, value });
});
  ...
  if (!init?.method || init.method === "GET") {
    ...
    if (typeof window !== "undefined") {
      try {
        const shared = JSON.parse(
          localStorage.getItem(`stocks.api.${url}`) || "null",
        );
        if (shared?.expires > Date.now()) {
          GET_CACHE.set(url, shared);
    ...
    const request = fetch(url, init)
      .then(async (resp) => {
        ...
        if (typeof window !== "undefined") {
          try {
            localStorage.setItem(`stocks.api.${url}`, JSON.stringify(cached));
          } catch {}
        }
...
export const api = {
  ...
  financials: (symbol: string, periodType: "annual" | "quarterly") =>
    jfetch<{ data: any[] }>(
      `/api/stocks/${symbol}/financials?period_type=${periodType}`,
    ),
  ratios: (symbol: string) =>
    jfetch<{ data: any[] }>(`/api/stocks/${symbol}/ratios`),
  technicals: (symbol: string) =>
    jfetch<{ technicals: Technicals | null }>(
      `/api/stocks/${symbol}/technicals`,
    ),
  newsFeed: (sentiment = "all", source?: string, days = 7, refresh?: number) =>
    jfetch<{ data: any[]; sources: string[] }>(
      `/api/news/feed?sentiment=${sentiment}&days=${days}${source ? "&source=" + encodeURIComponent(source) : ""}${refresh ? "&refresh=" + refresh : ""}`,
    ),
  news: (symbol: string, refresh?: number) =>
    jfetch<{ data: any[] }>(
      `/api/stocks/${symbol}/news${refresh ? "?refresh=" + refresh : ""}`,
    ),
  ...
  aiWatchlistRefresh: () =>
    jfetch<any>("/api/ai-watchlist/refresh", { method: "POST" }),
  notificationStatus: () =>
    jfetch<any>("/api/recommendations/notifications/status"),
  strategyPerformance: (
    strategy: string,
    period: string,
    method?: string,
    ...
    jfetch<any>(
      `/api/recommendations/strategy-performance/${strategy}?period=${period}${method ? `&method=${method}` : ""}${date ? `&date=${date}` : ""}`,
    ),
  recommendationsToday: () => jfetch<any>("/api/recommendations/today"),
  recommendationsScreener: (strategy: string, method?: string) =>
    jfetch<any>(
      `/api/recommendations/screener/${strategy}${method ? `?method=${method}` : ""}`,
    ),
  recommendationsStrategy: (strategy: string, method?: string) =>
    jfetch<any>(
      `/api/recommendations/strategy/${strategy}${method ? `?method=${method}` : ""}`,
    ),
  stockTradeIdeas: (symbol: string) =>
  ...
  strategyPerformanceTrades: (
    ...
    jfetch<any>(
      `/api/recommendations/strategy-performance/${strategy}/trades?period=${period}${method ? `&method=${method}` : ""}${date ? `&date=${date}` : ""}`,
    ),
  marketCalendar: (start: string, end: string) =>
    jfetch<any>(`/api/market/calendar?start_date=${start}&end_date=${end}`),
  marketHolidays: () => jfetch<any>("/api/market/holidays"),
  marketEvents: (start: string, end: string) =>
    jfetch<any>(`/api/market/events?start_date=${start}&end_date=${end}`),
  aiAutoTradeStatus: () => jfetch<any>("/api/ai-auto-trade/status"),
  aiAutoTradeRuns: () => jfetch<any[]>("/api/ai-auto-trade/runs"),
  aiAutoTradeConfig: (config: Record<string, unknown>) =>
    jfetch<any>("/api/ai-auto-trade/config", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(config),
    }),
  aiAutoTradeRun: () =>
    jfetch<any>("/api/ai-auto-trade/run", { method: "POST" }),
  ...
  aiTradingAnalysis: (ticker: string) =>
    jfetch<any>(`/api/ai-trading/analysis/${ticker}`),
  aiTradingBatches: () => jfetch<any[]>("/api/ai-trading/batches"),
  aiTradingBatchStart: (batch_size: number) =>
    jfetch<any>("/api/ai-trading/batches", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ batch_size }),
    }),
  aiTradingBatchResume: (id: number) =>
    jfetch<any>(`/api/ai-trading/batches/${id}/resume`, { method: "POST" }),
  aiTradingAnalyze: (
    ticker: string,
    quick_model?: string,
    deep_model?: string,
  ) =>
    jfetch<any>(`/api/ai-trading/analyze/${ticker}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ quick_model, deep_model }),
    }),
  paperSummary: () => jfetch<PaperSummary>("/api/paper-trading/summary"),
  paperPositions: () =>
    jfetch<{ data: PaperTrade[] }>("/api/paper-trading/positions"),
  paperHistory: () =>
    jfetch<{ data: PaperTrade[] }>("/api/paper-trading/history"),
  paperLogs: () =>
    jfetch<{ data: PaperLog[] }>("/api/paper-trading/logs?limit=50"),
  paperCandidates: () =>
    jfetch<{ data: PaperCandidate[] }>("/api/paper-trading/candidates"),
  paperToggle: (enabled: boolean) =>
    jfetch<{ paper_only: true; enabled: boolean }>(
  ...
  paperRun: () =>
    jfetch<{ status: string; created?: number }>("/api/paper-trading/run", {
      method: "POST",
    }),
  screener: (params: URLSearchParams) =>
    jfetch<{ data: any[]; generated_at?: string; market?: any }>(
      `/api/screener?${params.toString()}`,
    ),
  watchlist: () => jfetch<{ data: any[] }>("/api/watchlist"),
  watchlistAdd: (symbol: string, note?: string) =>
    jfetch("/api/watchlist", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "add", symbol, note }),
    }),
  ...
  watchlistUpdate: (symbol: string, note: string) =>
    jfetch("/api/watchlist", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "update", symbol, note }),
    }),
  watchlistReorder: (symbols: string[]) =>
    jfetch("/api/watchlist", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "reorder", symbols }),
    }),
  aiModels: () => jfetch<AIConfig[]>("/api/ai/models"),
  aiQueue: () =>
    jfetch<{ active: boolean; queued: number; max_concurrency: number }>(
      "/api/ai/queue-status",
    ),

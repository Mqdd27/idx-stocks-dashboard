...
export const api = {
  ...
  strategyPerformance: (strategy: string, period: string, method?: string) => jfetch<any>(`/api/recommendations/strategy-performance/${strategy}?period=${period}${method ? `&method=${method}` : ""}`),

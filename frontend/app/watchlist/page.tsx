...
function alertHits(item: WatchlistItem, alert?: Alert): string[] {
  ...
  return [
    alert?.price != null && price != null && price >= alert.price
      ? `Harga ≥ ${fmtNum(alert.price)}`
      : "",
    alert?.volume != null && (item.price?.volume || 0) >= alert.volume
      ? `Volume ≥ ${fmtVol(alert.volume)}`
      : "",
  ].filter(Boolean);
}
      ...
      .then((response) => {
        ...
        for (const item of response.data) {
          ...
          if (price != null && previous != null && price !== previous)
            nextFlashes[item.symbol] = price > previous ? "up" : "down";
    ...
    return (
      <div>
        ...
        <div className="empty-state card">
          Watchlist kosong. Buka halaman saham lalu klik "Add to Watchlist"
          untuk menambahkan.
        </div>
      </div>
    );
    ...
    <div>
      ...
      <div className="card">
        <div className="muted" style={{ marginBottom: 8 }}>
          Tarik baris untuk mengurutkan. Alert hanya aktif di browser ini saat
          halaman Watchlist terbuka.
        </div>
            ...
            <tbody>
              {items.map((item) => (
                <WatchlistRow
                  key={item.symbol}
                  item={item}
                  alert={alerts[item.symbol]}
                  hits={hits[item.symbol]}
                  flash={flashes[item.symbol]}
                  editing={editing === item.symbol}
                  note={note}
                  onDragStart={() => {
                    dragged.current = item.symbol;
                  }}
                  onDrop={() => reorder(item.symbol)}
                  onEdit={() => {
                    setEditing(item.symbol);
                    setNote(item.note || "");
                  }}
                  onNote={setNote}
                  onSaveNote={() => saveNote(item.symbol)}
                  onAlert={saveAlert}
                  onRemove={async () => {
                    await api.watchlistRemove(item.symbol);
                    load();
                  }}
                />
              ))}
...
function WatchlistRow({
  ...
  onSaveNote: () => void;
  onAlert: (symbol: string, alert: Alert) => void;
  onRemove: () => void;
}) {
  return (
    <tr
    ...
    >
      ...
      <td>
        <Link href={`/stock/${item.symbol}`} className="sym-badge">
          {item.symbol}
        </Link>
        {hits && <div className="neg">{hits.join(" · ")}</div>}
      </td>
      <td className={`num ${flash ? `flash-${flash}` : ""}`}>
        {item.price ? fmtNum(item.price.close) : "-"}
      </td>
      <td className={`num ${cls(item.price?.change_pct)}`}>
        {item.price ? pct(item.price.change_pct) : "-"}
      </td>
      <td className="num">{item.price ? fmtVol(item.price.volume) : "-"}</td>
      <td className="num">
        {item.ratios?.per != null ? fmtNum(item.ratios.per) : "-"}
      </td>
      <td>
        {editing ? (
          <>
            <input
              aria-label={`Catatan ${item.symbol}`}
              value={note}
      ...
      <td>
        <input
          aria-label={`Alert harga ${item.symbol}`}
          type="number"
          placeholder="Harga"
          ...
          onBlur={(event) =>
            onAlert(item.symbol, {
              ...alert,
              price: Number(event.target.value) || undefined,
            })
          }
        />
        <input
          aria-label={`Alert volume ${item.symbol}`}
          type="number"
          placeholder="Volume"
          defaultValue={alert?.volume || ""}
          onBlur={(event) =>
            onAlert(item.symbol, {
              ...alert,
              volume: Number(event.target.value) || undefined,
            })
          }
        />
      </td>
      ...
      >
        {item.latest_news ? (
          <a
            href={item.latest_news.url}
            target="_blank"
            rel="noopener noreferrer"
          >
            {item.latest_news.title}
          </a>
        ) : (

...
export default function StockDetailPage() {
  ...
  useEffect(() => {
    ...
    api
      .technicals(symbol)
      .then((t) => alive && setTech(t.technicals))
      .catch(() => {});
    api
      .watchlist()
      .then(
        (w) => alive && setInWatchlist(w.data.some((x) => x.symbol === symbol)),
      )
      .catch(() => {});
    return () => {
      alive = false;
  ...
  return (
    <div>
      <div className="stock-header">
        <div>
          <h2>
            {stock.symbol} <span className="name">· {stock.company_name}</span>
          </h2>
      ...
      {watchlistError && (
        <div className="analysis-err" role="alert">
          {watchlistError}
        </div>
      )}
      {tab === "overview" && (
...
function TradeIdeasTab({ symbol }: { symbol: string }) {
  ...
  useEffect(() => {
    api
      .stockTradeIdeas(symbol)
      .then(setData)
      .catch(() => setData({ data: [] }));
  }, [symbol]);
  if (!data)
    return <div className="terminal-loading">LOADING TRADE IDEAS...</div>;
  return (
    <div className="trade-ideas-detail">
      <div className="terminal-page-meta">
        <span>
          DATA AS OF{" "}
          {data.market?.current_time
            ? new Date(data.market.current_time).toLocaleString("id-ID")
            : "-"}
        </span>
        <span>MARKET {data.market?.status || "-"}</span>
      </div>
      {data.data?.length ? (
        <div className="trade-idea-detail-grid">
          {data.data.map((idea: any) => (
            <div className="terminal-panel" key={idea.id}>
              <header className="terminal-panel-head">
                <div>
                  <span className="terminal-panel-code">
                    {idea.method === "TRADING_AGENTS" ? "TA" : "QNT"}
                  </span>
                  <h2>
                    {idea.strategy} · {idea.action}
                  </h2>
                </div>
              </header>
              <div className="trade-idea-detail-body">
                <div className="stat-strip">
                  <Stat
                    label="Entry Zone"
                    value={`${fmtPrice(idea.entry_low)}–${fmtPrice(idea.entry_high)}`}
                  />
                  <Stat label="TP1" value={fmtPrice(idea.tp1)} />
                  <Stat label="TP2" value={fmtPrice(idea.tp2)} />
                  <Stat label="Stop Loss" value={fmtPrice(idea.stop_loss)} />
                  <Stat
                    label="R/R"
                    value={
                      idea.risk_reward
                        ? Number(idea.risk_reward).toFixed(2)
                        : "-"
                    }
                  />
                  <Stat label="Score" value={idea.score ?? "-"} />
                </div>
                <h3 className="card-title">WHY THIS IDEA</h3>
                <div className="trade-reason-list">
                  {(idea.reasons?.positive || []).map((x: string) => (
                    <div className="positive" key={x}>
                      + {x}
                    </div>
                  ))}
                  {(idea.reasons?.negative || []).map((x: string) => (
                    <div className="negative" key={x}>
                      − {x}
                    </div>
                  ))}
                  {idea.reasons?.decision && <p>{idea.reasons.decision}</p>}
                </div>
                <p className="muted">
                  Valid until:{" "}
                  {idea.valid_until
                    ? new Date(idea.valid_until).toLocaleString("id-ID")
                    : "-"}
                </p>
              </div>
            </div>
        ...
        <div className="terminal-empty">NO TRADE IDEAS FOR {symbol}</div>
      )}
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ marginBottom: 14 }}>
        <button
          className={`btn btn-sm ${inWatchlist ? "" : "btn-primary"}`}
          onClick={onWatchlist}
        >
      ...
      <div className="stat-strip">
        ...
        <Stat
          label="Change"
          value={p ? pct(p.change_pct) : "-"}
          color={cls(p?.change_pct)}
        />
      ...
      {tech ? (
        <div className="stat-strip">
          <Stat
            label="SMA 5"
            value={tech.sma5 != null ? fmtNum(tech.sma5) : "-"}
          />
          <Stat
            label="SMA 20"
            value={tech.sma20 != null ? fmtNum(tech.sma20) : "-"}
          />
          <Stat
            label="SMA 50"
            value={tech.sma50 != null ? fmtNum(tech.sma50) : "-"}
          />
          <Stat
            label="SMA 200"
            value={tech.sma200 != null ? fmtNum(tech.sma200) : "-"}
          />
          <Stat
            label="RSI 14"
            value={tech.rsi14 != null ? fmtNum(tech.rsi14) : "-"}
          />
          <Stat label="MACD" value={tech.macd ? fmtNum(tech.macd.macd) : "-"} />
          <Stat
            label="ATR 14"
            value={tech.atr14 != null ? fmtNum(tech.atr14) : "-"}
          />
          <Stat
            label="52W High"
            value={tech.high_52w != null ? fmtNum(tech.high_52w) : "-"}
          />
          <Stat
            label="52W Low"
            value={tech.low_52w != null ? fmtNum(tech.low_52w) : "-"}
          />
          <Stat
            label="Support"
            value={
              tech.support_resistance
                ? fmtNum(tech.support_resistance.support)
                : "-"
            }
          />
          <Stat
            ...
            value={
              tech.support_resistance
                ? fmtNum(tech.support_resistance.resistance)
                : "-"
            }
          />
          <Stat
        ...
        <div className="empty-state card">
          Data harga belum cukup untuk kalkulasi teknikal.
        </div>
      )}
      <h3 className="card-title" style={{ margin: "20px 0 10px" }}>
        Company
  ...
  return (
    <div>
      <div className="chart-toolbar" style={{ marginBottom: 10 }}>
        <button
          className={periodType === "annual" ? "active" : ""}
      ...
      {!loaded ? (
        ...
        <div className="empty-state card">
          Data laporan keuangan belum tersedia. Kolektor fundamental berjalan
          harian; IDX official API diblokir dari server ini (403), data diambil
          dari Yahoo Finance fundamentals.
        </div>
      ) : (
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i}>
                  <td>{r.period}</td>
                  <td className="num">
                    {r.revenue != null ? fmtBig(r.revenue) : "-"}
                  </td>
                  <td
                    className={`num ${r.net_income != null && r.net_income >= 0 ? "pos" : "neg"}`}
                  >
                    {r.net_income != null ? fmtBig(r.net_income) : "-"}
                  </td>
                  <td className="num">
                    {r.total_assets != null ? fmtBig(r.total_assets) : "-"}
                  </td>
                  <td className="num">
                    {r.total_liabilities != null
                      ? fmtBig(r.total_liabilities)
                      : "-"}
                  </td>
                  <td className="num">
                    {r.total_equity != null ? fmtBig(r.total_equity) : "-"}
                  </td>
                  <td className="num">
                    {r.operating_cashflow != null
                      ? fmtBig(r.operating_cashflow)
                      : "-"}
                  </td>
                  <td className="num">
                    {r.capex != null ? fmtBig(r.capex) : "-"}
                  </td>
                </tr>
              ))}
            </tbody>
...
function ValuationTab({ symbol }: { symbol: string }) {
  ...
  if (rows.length === 0)
    return (
      <div className="empty-state card">
        Rasio valuasi belum tersedia (fundamental belum tersinkron).
      </div>
    );
      ...
      <table>
        <thead>
          <tr>
            ...
            <th className="num">NPM</th>
            <th className="num">Gross Mgn</th>
            <th className="num">Div Yield</th>
            <th className="num">Rev Growth</th>
            <th className="num">NI Growth</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={i}>
              ...
              <td className="num">
                {r.roe != null ? fmtNum(r.roe) + "%" : "-"}
              </td>
              <td className="num">
                {r.roa != null ? fmtNum(r.roa) + "%" : "-"}
              </td>
              <td className="num">{r.der != null ? fmtNum(r.der) : "-"}</td>
              <td className="num">
                {r.npm != null ? fmtNum(r.npm) + "%" : "-"}
              </td>
              <td className="num">
                {r.gross_margin != null ? fmtNum(r.gross_margin) + "%" : "-"}
              </td>
              <td className="num">
                {r.dividend_yield != null
                  ? fmtNum(r.dividend_yield) + "%"
                  : "-"}
              </td>
              <td className={`num ${cls(r.revenue_growth)}`}>
                {r.revenue_growth != null
                  ? fmtNum(r.revenue_growth) + "%"
                  : "-"}
              </td>
              <td className={`num ${cls(r.net_income_growth)}`}>
                {r.net_income_growth != null
                  ? fmtNum(r.net_income_growth) + "%"
                  : "-"}
              </td>
            </tr>
          ))}
  ...
  return (
    <div>
      ...
      {!loaded ? (
        ...
        <div className="empty-state card">
          Belum ada berita tersedia untuk {symbol}.
        </div>
      ) : (
        <div className="card">
          {rows.map((n, i) => (
            <div className="news-item" key={i}>
              ...
              <div className="m">
                {n.source} ·{" "}
                {n.published_at
                  ? new Date(n.published_at).toLocaleString("id-ID")
                  : ""}
              </div>
              {n.summary && (
...
function AnalyzeTab({ symbol }: { symbol: string }) {
  ...
  const [fb, setFb] = useState<{ message: string; provider: string } | null>(
    null,
  );
  ...
  async function analyze(overrideModel?: string) {
    ...
    try {
      await api.aiAnalyzeStream(
        ...
        (msg, provider) =>
          setFb({ message: msg, provider: provider || "unknown" }),
      );
    } catch (e: any) {
    ...
    <div>
      <div className="card">
        ...
        <div className="muted" style={{ marginTop: 10, fontSize: 11 }}>
          Analisis dijalankan hanya saat tombol ditekan. Data snapshot: harga,
          teknikal, fundamental, aksi korporasi & berita terbaru.
        </div>
      </div>
      {busy && (
      ...
      {fb && (
        <div className="analysis-err">
          ...
          <div style={{ fontSize: 12, color: "var(--muted)", margin: "8px 0" }}>
            Fallback available: cloud model
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button
              className="btn btn-sm"
              onClick={() => analyze(model)}
              disabled={busy}
            >
              Retry Local
            </button>
            <button
              ...
              onClick={() => api.cloudFallbackModel().then((m) => analyze(m))}
              disabled={busy}
            >
              Use Cloud
            </button>
      ...
      {result && (
        <div className="analysis-box" style={{ marginTop: 14 }}>
          ...
          <div className="analysis-meta">
            Analyzed using: <strong>{result.model}</strong> (
            {result.provider === "ollama" ? "LOCAL" : "CLOUD via 9Router"})
            <br />
            Generated at:{" "}
    ...
    <div className="markdown-output">
      {text.split("\n").map((line, index) => {
        ...
        if (/^#{1,6}\s/.test(value))
          return (
            <h3 key={index}>
              {inlineMarkdown(value.replace(/^#{1,6}\s/, ""))}
            </h3>
          );
        if (/^[-*]\s/.test(value))
          return (
            <div className="markdown-item" key={index}>
              • {inlineMarkdown(value.slice(2))}
            </div>
          );

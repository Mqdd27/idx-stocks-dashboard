...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    api.aiModels().then((m) => { setModels(m); setSelectedModel(getModel() || m[0]?.id || ""); }).catch(() => {});
    ...
    const queueTimer = setInterval(() => api.aiQueue().then(setQueue).catch(() => {}), 5000);
    return () => { clearInterval(timer); clearInterval(queueTimer); };
  }, []);
  ...
  const tradingModels = (status?.available_models || []).filter((m: string) => m.startsWith("cx/") || m.startsWith("gemini/") || m.startsWith("openai/"));
  ...
  return <div>
    ...
    <div className="grid grid-2">
      <div className="card">
        ...
        <label style={{ display: "flex", gap: 8, alignItems: "center" }}><input type="checkbox" checked={fallback} onChange={(e) => { setFallback(e.target.checked); }} /> Allow AI fallback</label>
      </div>
      <div className="card">
    ...
    <div className="grid grid-2" style={{ marginTop: 24, marginBottom: 14 }}>
      <div className="card"><div className="card-title">Preferred AI Model</div><select className="ai-select" style={{ width: "100%", maxWidth: "none" }} value={selectedModel} onChange={(e) => { setSelectedModel(e.target.value); setModel(e.target.value); }}>{models.filter((m) => m.local).length > 0 && <optgroup label="LOCAL">{models.filter((m) => m.local).map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</optgroup>}{models.filter((m) => !m.local).length > 0 && <optgroup label="CLOUD">{models.filter((m) => !m.local).map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</optgroup>}</select><div className="muted" style={{ marginTop: 8, fontSize: 11 }}>Model untuk Analyze, Chat, Summarize, dan Explain Financials.</div></div>
      <div className="card"><div className="card-title">Local AI Queue</div>{queue ? <><div>Active: <strong>{queue.active ? "Yes" : "No"}</strong></div><div>Queued: <strong>{queue.queued}</strong></div><div>Max concurrency: <strong>{queue.max_concurrency}</strong></div></> : <div className="muted">Memuat…</div>}<label style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 10, fontSize: 13 }}><input type="checkbox" checked={fallback} onChange={(e) => { setFallbackState(e.target.checked); setFallback(e.target.checked); }} /> Allow AI fallback</label></div>
    </div>
    <div className="card"><div className="card-title">System Health</div><div className="stat-strip"><div className="stat"><div className="label">Backend</div><div className="value">{health ? health.status : "-"}</div></div><div className="stat"><div className="label">Database</div><div className="value">{health ? health.database : "-"}</div></div><div className="stat"><div className="label">9Router</div><div className="value">{health ? health.nine_router : "-"}</div></div><div className="stat"><div className="label">Ollama</div><div className="value">{health ? health.ollama : "-"}</div></div></div></div>
    ...
    <div className="card auto-actions"><div><strong>Manual analysis</strong><p className="muted">Runs asynchronously and does not open a trade.</p></div><div className="action-buttons"><select className="ai-select ai-ticker" aria-label="IDX stock" value={ticker} onChange={(e) => setTicker(e.target.value)}><option value="">Select stock</option>{stocks.map((stock: any) => <option key={stock.symbol} value={stock.symbol}>{stock.symbol} — {stock.company_name}</option>)}</select><select className="ai-select" aria-label="Quick model" value={quick} onChange={(e) => setQuick(e.target.value)}>{tradingModels.map((m: string) => <option key={m}>{m}</option>)}</select><select className="ai-select" aria-label="Deep model" value={deep} onChange={(e) => setDeep(e.target.value)}>{tradingModels.map((m: string) => <option key={m}>{m}</option>)}</select><button className="btn btn-primary" disabled={!status?.enabled || !ticker} onClick={run}>Run analysis</button></div></div>

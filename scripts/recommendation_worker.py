...
from app.recommendation_service import (
    generate_quant,
    generate_tradingagents_shortlist,
    update_outcomes,
)
...
def notify_telegram(strategy, method):
        ...
        rows = (
            db.execute(
                ...
                .where(
                    ...
                    TradeRecommendation.method == method,
    ...
    for row in rows:
        lines.append(
            f"{row.symbol} | Entry {round(row.entry_low or 0)}-{round(row.entry_high or 0)} | TP {round(row.tp1 or 0)} | SL {round(row.stop_loss or 0)} | Score {row.score or '-'}"
        )
    subprocess.run(
        ["/home/mqdd/.local/bin/hermes", "send", "--to", "telegram", "\n".join(lines)],
        check=False,
        timeout=30,
    )
...
print(
    f"RECOMMENDATION_WORKER_START date={now.date()} status={market['status']}",
    flush=True,
)
if now.hour == 22:
    print(
        "PAPER_BPJS_PREVIEW",
        generate_quant("BPJS", cycle="night-preview", preview=True),
        flush=True,
    )
    print(
        "TA_BPJS",
        generate_tradingagents_shortlist("BPJS", cycle="night-preview", now=now),
        flush=True,
    )
...
print(
    "TA_SHORTLIST",
    generate_tradingagents_shortlist("GENERAL" if now.hour == 9 else "BSJP"),
    flush=True,
)
print("WATCHLIST_GENERATE", generate_watchlist(now), flush=True)
print("WATCHLIST_STATUS", refresh_status(now), flush=True)

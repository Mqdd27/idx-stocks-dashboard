    ...
    for index, row in enumerate(rows[:3], 1):
        lines.extend(
            [
                ...
                (
                    f"{'Confidence' if ai else 'Quant Score'}: {row.confidence_label or '-'}"
                    if ai
                    else f"Quant Score: {row.score or '-'}"
                ),
            ]
        )
        if not ai:
            ...
            risks = (
                (row.reasons or {}).get("negative")
                or (row.risks or {}).get("items")
                or []
            )
            if positives:
                lines.append("Why:")
...
def _records(strategy):
    with SessionLocal() as db:
        recs = (
            db.execute(
                ...
                .order_by(
                    desc(TradeRecommendation.trading_date),
                    desc(TradeRecommendation.generated_at),
                    desc(TradeRecommendation.id),
                )
            )
            .scalars()
            .all()
    ...
    paper = _latest(
        [row for row in recs if row.method == "PAPER_TRADE" and row.action == "BUY"]
    )
    agents = _latest(
        [row for row in recs if row.method == "TRADING_AGENTS" and row.action == "BUY"]
    )
    latest_date = max(
        [row.trading_date for row in paper + agents], default=datetime.now(TZ).date()
    )
    return (
        latest_date,
        [row for row in agents if row.trading_date == latest_date],
        [row for row in paper if row.trading_date == latest_date],
    )


def render_report_image(strategy, path=None):
    trading_date, agents, paper = _records(strategy)
    rows = [("TRADINGAGENTS", row) for row in agents[:5]] + [
        ("PAPER ENGINE", row) for row in paper[:5]
    ]
    width, row_h = 1400, 68
    ...
    amber, text, muted, green, red, border = (
        "#f6a623",
        "#e8ecf4",
        "#7d879c",
        "#00c176",
    ...
    draw.text(
        (42, 90),
        f"REKOMENDASI SAHAM · {trading_date.strftime('%d %b %Y')}",
        font=bold,
        fill=amber,
    )
    draw.text(
        (42, 130),
        "Source separated: TradingAgents / Paper Trade Quant Engine",
        font=small,
        fill=muted,
    )
    columns = [
    ...
    for source, row in rows:
        ...
        values = [
            source,
            row.symbol,
            f"{_price(row.entry_low)}-{_price(row.entry_high)}",
            f"{_price(row.tp1)} / {_price(row.tp2)}",
            _price(row.stop_loss),
            f"{float(row.risk_reward or 0):.2f}",
            (
                row.confidence_label
                if source == "TRADINGAGENTS"
                else str(row.score or "-")
            ),
            row.status,
        ]
        for (x, _), value in zip(columns, values):
            draw.text(
                (x, y + 18),
                str(value),
                font=bold if value == row.symbol else regular,
                fill=color if value in (source, row.symbol) else text,
            )
        y += row_h
    draw.text(
        (42, height - 65),
...
def build_report(strategy, now=None):
    ...
    target = (
        ...
        if strategy == "BSJP" or now.hour >= 16
        else now.date()
    )
    with SessionLocal() as db:
        recs = (
        ...
        agents = _latest(
            [
                row
                for row in recs
                if row.method == "TRADING_AGENTS" and row.action == "BUY"
            ]
        )
        watch = (
            db.execute(
    ...
    lines = [
        ...
        *([f"{symbol} — Consensus: STRONG" for symbol in overlap] or ["NO CONSENSUS"]),
        ...
        divider,
        *(
            [f"{row.symbol} — {row.status}" for row in watch_latest[:5]]
            or ["NO WATCHLIST CANDIDATES"]
        ),
        "",
        divider,
        "⚡ BSJP / BPJS",
        divider,
        *(
            [f"{row.symbol} — {strategy} / {row.method}" for row in agents[:5]]
            or [f"{strategy} — NO TRADINGAGENTS PICK"]
        ),
        "",
        "Research / paper trading only. No guaranteed profit; no brokerage order.",
    ]
...
def send_report(strategy):
    ...
    with SessionLocal() as db:
        delivery = db.execute(
            ...
            .where(
                TelegramDelivery.message_type == "recommendation",
                TelegramDelivery.target_date == target_date,
                TelegramDelivery.cycle == strategy,
            )
            .with_for_update()
        ).scalar_one_or_none()
        if (
            ...
            and delivery.content_hash == content_hash
        ):
            return 0
        if not delivery:
    ...
    try:
        ...
        text_result = subprocess.run(
            ["/home/mqdd/.local/bin/hermes", "send", "--to", "telegram", message],
            check=False,
            timeout=45,
        )
        ...
        error = (
            None
            if ok
            else f"media={media_result.returncode},text={text_result.returncode}"
        )
    ...
    with SessionLocal() as db:
        ...
        delivery.sent_at = datetime.now(TZ) if ok else None
        db.commit()

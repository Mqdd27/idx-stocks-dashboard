...
class SecurityTimezoneTest(unittest.TestCase):
    def setUp(self):
        ...
        os.environ["CORS_ALLOWED_ORIGINS"] = (
            "https://stocks.mqdd.my.id,http://localhost:3100"
        )
    ...
    def test_public_read_and_protected_write(self):
        ...
        self.assertEqual(
            self.client.post(
                "/api/ai-trading/batches", json={"batch_size": 1}
            ).status_code,
            401,
        )
        self.assertEqual(
            self.client.post(
                "/api/ai-trading/batches",
                json={"batch_size": 1},
    ...
    def test_cors_allowlist(self):
        ...
        good = self.client.options(
            "/api/stocks",
            headers={
                "Origin": "https://stocks.mqdd.my.id",
                "Access-Control-Request-Method": "GET",
            },
        )
        self.assertNotEqual(
        ...
        self.assertEqual(
            good.headers.get("access-control-allow-origin"), "https://stocks.mqdd.my.id"
        )

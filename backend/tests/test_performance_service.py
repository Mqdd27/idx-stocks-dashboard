...
class PerformanceServiceTest(unittest.TestCase):
    ...
    def test_period_bounds(self):
        self.assertEqual(
            period_bounds("weekly", date(2026, 9, 4)),
            (date(2026, 8, 31), date(2026, 9, 6)),
        )
        self.assertEqual(
            period_bounds("monthly", date(2026, 9, 4))[0], date(2026, 9, 1)
        )
    ...
    def test_bpjs_overnight_anomaly_excluded(self):
        result = _metrics(
            [self.row("BPJS", exit_date="2026-09-07")],
            "BPJS",
            "PAPER_TRADE",
            "daily",
            date(2026, 9, 4),
    ...
    def test_metrics_and_exclusions(self):
        rows = [
            self.row(value=10, exit_date="2026-09-04"),
            self.row(value=-5, exit_date="2026-09-04"),
            self.row(value=0, exit_date="2026-09-04"),
            self.row(status="ACTIVE", value=4, exit_date=None),
            self.row(status="EXPIRED", entered=False, value=None, exit_date=None),
        ]
        result = _metrics(
            rows, "BSJP", "PAPER_TRADE", "daily", date(2026, 9, 4), date(2026, 9, 4)
        )
        ...
        self.assertEqual(result["not_triggered"], 1)
        self.assertAlmostEqual(result["profit_factor"], 2)
        self.assertAlmostEqual(result["net_return_pct"], 4.5)

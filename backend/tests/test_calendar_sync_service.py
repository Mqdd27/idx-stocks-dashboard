...
class CalendarSyncServiceTest(unittest.TestCase):
    def test_primary_url_is_preferred_when_configured(self):
        with patch.dict(
            "os.environ", {"IDX_CALENDAR_URL": "https://idx.example/{year}"}
        ):
            with patch.object(
                calendar_sync_service, "_fetch", return_value=[{"date": "2026-01-01"}]
            ):
                with patch.object(calendar_sync_service, "SessionLocal") as session:
                    session.return_value.execute.return_value.scalar_one_or_none.return_value = (
                        None
                    )
        ...
        with patch.dict(
            ...
            with patch.object(calendar_sync_service, "_fetch", side_effect=fetch):
                with patch.object(calendar_sync_service, "SessionLocal") as session:
                    ...
                    result = calendar_sync_service.sync_calendar(2026)
        self.assertEqual(
            result["source"], "Nager.Date Indonesia public holiday fallback"
        )

...
class PaperTradingTest(unittest.TestCase):
    ...
    def test_lot_accounting_and_dynamic_setup_score(self):
        ...
        self.assertGreater(
            setup_confidence(4, 110, 100, 90, 120),
            setup_confidence(4, 95, 100, 90, 120),
        )
        self.assertEqual(setup_confidence(4, 120, 100, 90, 120), 4)

    def test_exits_and_metrics(self):
        self.assertEqual(
            check_exit(date(2026, 1, 1), date(2026, 1, 2), 90, 95, 110, 20).reason,
            "stop_loss",
        )
        self.assertEqual(
            check_exit(date(2026, 1, 1), date(2026, 1, 21), 100, 95, 110, 20).reason,
            "time_exit",
        )

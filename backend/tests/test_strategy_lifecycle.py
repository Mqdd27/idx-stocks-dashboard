...
class BSJPCrossDayTest(unittest.TestCase):
    ...
    def test_bsjp_counted_on_entry_date_only(self):
        row = self.row()
        res = _metrics(
            [row], "BSJP", "PAPER_TRADE", "daily", date(2026, 9, 4), date(2026, 9, 4)
        )
        self.assertEqual(res["trades"], 1)
        res = _metrics(
            [row], "BSJP", "PAPER_TRADE", "daily", date(2026, 9, 5), date(2026, 9, 5)
        )
    ...
    def test_bsjp_weekly_includes_entry_week(self):
        ...
        res = _metrics(
            [row], "BSJP", "PAPER_TRADE", "weekly", date(2026, 8, 31), date(2026, 9, 6)
        )
        self.assertEqual(res["trades"], 1)
    ...
    def test_bsjp_holiday_weekend_handled(self):
        ...
        self.assertEqual(res["trades"], 1)
        res = _metrics(
            [row], "BSJP", "PAPER_TRADE", "daily", date(2026, 9, 8), date(2026, 9, 8)
        )
        self.assertEqual(res["trades"], 0)
...
class BPJSOvernightAnomalyTest(unittest.TestCase):
    ...
    def test_bpjs_same_day_not_anomaly(self):
        ...
        res = _metrics(
            [row], "BPJS", "PAPER_TRADE", "daily", date(2026, 9, 4), date(2026, 9, 4)
        )
        self.assertEqual(res["trades"], 1)

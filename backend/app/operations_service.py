...
def operations_health(db):
    ...
    return {
        "generated_at": now,
        "calendar": {"fresh": calendar_fresh(), "last_sync": calendar.created_at if calendar else None, "source": (calendar.details or {}).get("source") if calendar else None},
        "collector": {"last_intraday": intraday.timestamp if intraday else None, "stale": not intraday or now - intraday.timestamp > timedelta(minutes=20)},
        "data_quality": data_quality(db, now),
        "batch": {"id": batch.id if batch else None, "status": batch.status if batch else "NONE", "completed": batch.completed if batch else 0, "failed": batch.failed if batch else 0, "total": batch.total if batch else 0, "updated_at": batch.updated_at if batch else None},
        "telegram": {"failed_count": len(failed), "failures": [{"id": row.id, "message_type": row.message_type, "target_date": row.target_date, "cycle": row.cycle, "last_error": row.last_error, "attempt_count": row.attempt_count} for row in failed]},
    }

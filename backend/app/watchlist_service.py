...
def _status(row, now):
    if row.status in ("EXPIRED", "SL_HIT", "TP2_HIT"):
        return "EXPIRED"
    ...
    with SessionLocal() as db:
        recs = (
            db.execute(
                ...
                .where(TradeRecommendation.trading_date == now.astimezone().date())
        ...
        for rec in recs:
            if rec.method not in ("TRADING_AGENTS", "PAPER_TRADE"):
                continue
            existing = db.execute(
                select(AIWatchlist).where(
                    AIWatchlist.trading_date == rec.trading_date,
                    AIWatchlist.symbol == rec.symbol,
                    AIWatchlist.method == rec.method,
                )
            ).scalar_one_or_none()
            if existing:
                continue
...
def refresh_status(now=None):
    ...
    with SessionLocal() as db:
        items = (
            db.execute(
                select(AIWatchlist).where(
                    AIWatchlist.status.in_(
                        ["WATCH", "WAIT_ENTRY", "NEAR_ENTRY", "READY"]
                    )
                )
            )
            .scalars()
        ...
        for item in items:
            rec = db.execute(
                select(TradeRecommendation)
                .where(
                    TradeRecommendation.trading_date == item.trading_date,
                    TradeRecommendation.symbol == item.symbol,
                    TradeRecommendation.method == item.method,
                )
                .order_by(desc(TradeRecommendation.id))
                .limit(1)
            ).scalar_one_or_none()
            if not rec:
                continue
            item.status = _status(rec, now)
            item.updated_at = now
            updated += 1
        db.commit()

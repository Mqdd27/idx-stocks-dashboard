...
def _atr_setup(db, symbol: str, action: str):
    ...
    if action != "BUY":
        return {
            "action": action,
            "note": "Setup beli tidak dibuat untuk sinyal selain BUY.",
            "entry": entry,
        }
    setup = build_quant_setup(
        entry, atr, entry - 1.5 * atr, None, min_rr=2.0, target_rr=3.0, zone_atr=0
    )
    if not setup:
        return None
    return {
        ...
        "risk_reward": round(
            (setup["tp2"] - setup["entry"]) / (setup["entry"] - setup["stop"]), 2
        ),
        "atr": round(atr, 2),
...
def _reasoning(result: dict | None):
    ...
    return [
        {
            "agent": "Analis Pasar / Teknikal",
            "content": _clean_report(
        ...
        {"agent": "Peneliti Bearish", "content": _clean_report(bear_text)},
        {
            "agent": "Manajer Riset",
            "content": _clean_report(
                reports.get("investment_plan_id") or reports.get("investment_plan")
            ),
        },
        {
            "agent": "Rekomendasi Trader & Setup",
            "content": _clean_report(
                reports.get("trader_investment_plan_id")
                or reports.get("trader_investment_plan")
            ),
        },
        {"agent": "Analisis Risiko", "content": _clean_report(judge_text)},
        {
            "agent": "Keputusan Akhir Portofolio",
            "content": _clean_report(
                reports.get("final_trade_decision_id")
...
def status():
    ...
    try:
        ...
        response = httpx.get(
            f"{settings.nine_router_url}/models", headers=headers, timeout=5
        )
    ...
    with SessionLocal() as db:
        last = db.execute(
            ...
            .order_by(desc(AITradingAnalysis.created_at))
            .limit(1)
        ).scalar_one_or_none()
    return {
        "enabled": settings.ai_trading_enabled,
        "tradingagents_installed": True,
        "nine_router_reachable": reachable,
        "quick_model": settings.ai_trading_quick_model,
        "deep_model": settings.ai_trading_deep_model,
...
def jobs():
    with SessionLocal() as db:
        rows = (
            db.execute(
                select(AITradingJob).order_by(desc(AITradingJob.created_at)).limit(50)
            )
            .scalars()
            .all()
        )
...
def latest(ticker: str):
    ...
    with SessionLocal() as db:
        row = db.execute(
            select(AITradingAnalysis)
            .where(AITradingAnalysis.symbol == symbol)
            .order_by(desc(AITradingAnalysis.created_at))
            .limit(1)
        ).scalar_one_or_none()
    if not row:
    ...
    return {
        ...
        "status": row.status,
        "setup": setup,
        "reasoning": [r for r in _reasoning(row.result) if r["content"]],
        "result": row.result,
    }
...
async def request_analysis(ticker: str, request: Request):
    ...
    with SessionLocal() as db:
        active = db.execute(
            select(AITradingJob)
            .where(AITradingJob.status.in_(["QUEUED", "RUNNING"]))
            .limit(1)
        ).scalar_one_or_none()
        if active:
    ...
    async def worker():
        with SessionLocal() as db:
            row = db.get(AITradingJob, job_id)
            row.status = "RUNNING"
            db.commit()
        try:

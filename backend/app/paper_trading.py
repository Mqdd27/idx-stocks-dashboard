...
def setup_confidence(
    ...
    if current_price <= stop_loss:
        ...
        progress = (
            (current_price - entry_price) / (take_profit - entry_price)
...
def trade_metrics(trades: list) -> dict:
    ...
    return {
        ...
        "profit_factor": (
            ...
            if losses
            else (None if not profits else float("inf"))
        ),
...
def decide(ind: dict, min_score: float = 3, min_rr: float = 2) -> PaperDecision:
    ...
    score, reasons = 0, []
    if ind.get("sma20") and price > ind["sma20"]:
        score += 1
        reasons.append("price above SMA20")
    if ind.get("sma50") and price > ind["sma50"]:
        score += 1
        reasons.append("price above SMA50")
    if ind.get("rsi14") is not None and 50 <= ind["rsi14"] <= 70:
        score += 1
        reasons.append("RSI confirms momentum")
    ...
    if score < min_score or rr < min_rr:
        return PaperDecision(
            "no_trade", score, stop, target, rr, "; ".join(reasons) or "filters not met"
        )

...
def _news_factory_without_optional(llm):
    from tradingagents.agents.utils.agent_utils import get_global_news, get_instrument_context_from_state, get_language_instruction, get_news
    def news_node(state):
        current_date=state["trade_date"]
        context=get_instrument_context_from_state(state)
        tools=[get_news,get_global_news]
        system=(f"You are a news researcher analyzing recent company and market news for trading. Use get_news for ticker-specific news and get_global_news for broader market news. Do not infer unavailable macro data. Return actionable evidence and a Markdown table."+get_language_instruction())
        prompt=ChatPromptTemplate.from_messages([("system","You are a helpful AI assistant collaborating with other assistants. You have access to: {tool_names}. Today's date is {current_date}. {instrument_context}\n{system_message}"),MessagesPlaceholder(variable_name="messages")])
        chain=prompt.partial(system_message=system,tool_names=", ".join(t.name for t in tools),current_date=current_date,instrument_context=context)|llm.bind_tools(tools)
        result=chain.invoke(state["messages"])
        return {"messages":[result],"news_report":result.content if not result.tool_calls else ""}
    return news_node
...
def _fetch_local_news(symbol: str) -> str:
    ...
    try:
        ...
        logger.warning("local news lookup failed symbol=%s error=%s", symbol, type(exc).__name__)
        return ""
...
def analyze(symbol, quick_model=None, deep_model=None):
    ...
    with _lock:
        models=_models(); quick=quick_model or s.ai_trading_quick_model; deep=deep_model or s.ai_trading_deep_model
        ...
        c=DEFAULT_CONFIG.copy(); c.update(llm_provider='openai_compatible',backend_url='http://127.0.0.1:20128/v1',quick_think_llm=quick,deep_think_llm=deep,max_debate_rounds=1,max_risk_discuss_rounds=1,llm_max_retries=0,request_timeout=60,max_recur_limit=40,output_language='Bahasa Indonesia',data_vendors={**DEFAULT_CONFIG.get('data_vendors',{}),'macro_data':'fred' if s.enable_fred_enrichment else 'disabled','prediction_markets':'polymarket' if s.enable_polymarket_enrichment else 'disabled'})

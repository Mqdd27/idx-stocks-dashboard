...
def _latest_records(records):
    ...
    for record in records:
        ...
        if current is None or (record.generated_at, record.id) > (
...
def _current_records(records):
    ...
    for record in records:
        key = (record.symbol, record.method, record.strategy)
        preview = (
            record.status == "PREVIEW" or "preview" in (record.cycle or "").lower()
        )
        ...
        existing_rank = (
            ...
            if existing is None
                ...
                (
                    ...
                    or "preview" in (existing.cycle or "").lower()
                    else 1
                ),
...
def _stored(strategy, method=None, symbol=None, trading_date=None):
    ...
    with SessionLocal() as db:
        query = select(TradeRecommendation).where(
            TradeRecommendation.trading_date == today,
            TradeRecommendation.strategy == strategy,
        )
        if method:
            query = query.where(TradeRecommendation.method == method)
        if symbol:
            query = query.where(TradeRecommendation.symbol == symbol.upper())
        rows = (
            db.execute(
                query.order_by(
                    desc(TradeRecommendation.generated_at), desc(TradeRecommendation.id)
                )
            )
            .scalars()
            .all()
        ...
        if not rows and strategy == "BPJS":
            ...
            query = select(TradeRecommendation).where(
                TradeRecommendation.trading_date < today,
                TradeRecommendation.strategy == strategy,
                TradeRecommendation.valid_until >= now,
                TradeRecommendation.status.in_(["ACTIVE", "PREVIEW"]),
            )
            if method:
                query = query.where(TradeRecommendation.method == method)
            if symbol:
            ...
            rows = (
                db.execute(
                    query.order_by(
                        ...
                        desc(TradeRecommendation.score),
                    )
                )
                .scalars()
                .all()
    ...
    return [
        ...
        for record in sorted(
            latest,
            key=lambda item: (float(item.score or 0), item.generated_at),
            reverse=True,
        )
    ]
...
def strategy(strategy: str, method: str | None = None, trading_date: str | None = None):
    ...
    return {
        ...
        "status": "OK" if data else "NO_TRADE",
...
def today(trading_date: str | None = None):
    ...
    with SessionLocal() as db:
        rows = (
            db.execute(
                ...
                .where(
                    TradeRecommendation.trading_date == d,
                    TradeRecommendation.action != "NO_TRADE",
                )
                .order_by(
                    desc(TradeRecommendation.generated_at), desc(TradeRecommendation.id)
                )
        ...
        for sym in symbols:
            ta_action = next(
                (
                    r.action
                    for r in rows
                    if r.symbol == sym and r.method == "TRADING_AGENTS"
                ),
                None,
            )
            pp_action = next(
                (
                    r.action
                    for r in rows
                    if r.symbol == sym and r.method == "PAPER_TRADE"
                ),
                None,
            )
            if ta_action and pp_action:
                consensus[sym] = (
                    "STRONG" if ta_action == pp_action == "BUY" else "MIXED"
                )
    ...
    return {
        ...
        "consensus": consensus,
        "no_trade": {
            "trading_agents": "NO QUALIFIED SETUP" if not ta else None,
            "paper_trade": "NO QUALIFIED SETUP" if not paper else None,
        },
    }
...
def ta_picks(trading_date: str | None = None):
    ...
    return {
        "trading_date": d,
        "data": _stored("GENERAL", "TRADING_AGENTS", trading_date=d),
        "generated_at": datetime.now(timezone.utc).isoformat(),
    }
...
def paper_picks(trading_date: str | None = None):
    ...
    return {
        "trading_date": d,
        "data": _stored("GENERAL", "PAPER_TRADE", trading_date=d),
        "generated_at": datetime.now(timezone.utc).isoformat(),
    }
    ...
    with SessionLocal() as db:
        rows = (
            db.execute(
                ...
                .order_by(desc(TelegramDelivery.generated_at))
                .limit(20)
            )
            .scalars()
            .all()
...
def strategy_performance(
    ...
    with SessionLocal() as db:
        return get_performance(
            ...
            strategy.upper(),
            period,
            date_type.fromisoformat(date) if date else None,
            method,
        )


@router.get("/strategy-performance/{strategy}/trades")
def strategy_performance_trades(
    strategy: str,
...
def stock_trade_ideas(symbol: str):
    data = (
        _stored("GENERAL", symbol=symbol)
        + _stored("BSJP", symbol=symbol)
        + _stored("BPJS", symbol=symbol)
    )
...
def screener_strategy_alias(strategy: str, method: str | None = None):
    ...
    return {
        ...
        "market": market,
        "status": "OK" if data else "NO_TRADE",
        "reason": None if data else "OUTSIDE_STRATEGY_WINDOW_OR_NO_QUALIFIED_SETUP",
        "generated_at": datetime.now(timezone.utc).isoformat(),
    }
...
def _row(r):
    return {
        ...
        "mode": (
            "PREVIEW"
            if r.status == "PREVIEW"
            or (r.strategy == "BPJS" and "preview" in (r.cycle or "").lower())
            else "LIVE"
        ),
        "action": r.action,
        "status": r.status,
